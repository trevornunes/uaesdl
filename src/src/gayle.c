...
{
    printf ("IDE%d drive multiple mode = %d\n", ide->num, ide_nsector);
...
{
    printf ("IDE%d set features %2.2X (%2.2X)\n", ide->num, ide_feat, ide_nsector);
    ide_fail();
}
...
{
    if (IDE_LOG > 0)
	printf ("IDE%d DATA read without DRQ!?\n", ide->num);
    return 0xffff;
}
...
{
    if (IDE_LOG > 0)
	printf ("IDE%d DATA write without DRQ!?\n", ide->num);
}

static int get_ide_reg (uaecptr addr)
{
  int cs_ide = currprefs.cs_ide;

    uaecptr a = addr;
    addr &= 0xffff;
    if (addr >= 0x3020 && addr <= 0x3021 && cs_ide == 2)
	return -1;
    addr &= ~0x2020;
    addr >>= 2;
    ide2 = 0;
...
static void ide_write (uaecptr addr, uae_u32 val)
{
    ...
    ide_devcon &= ~0x80;
    if (IDE_LOG > 1 && addr != 0x2000 && addr != 0x2001 && addr != 0x2020 && addr != 0x2021 && addr != GAYLE_IRQ_1200)
	printf ("IDE_WRITE %08.8X=%02.2X PC=%X\n", addr, (uae_u32)val & 0xff, m68k_getpc ());
    if (currprefs.cs_ide <= 0)
	return;
    if (currprefs.cs_ide == 1) {
	if (addr == GAYLE_IRQ_1200) {
...
static int gayle_read (uaecptr addr)
{
    ...
    v = ide_read(addr);
    if (GAYLE_LOG)
	printf ("GAYLE_READ %08.8X=%02.2X PC=%08.8X\n", oaddr, (uae_u32)v & 0xff, m68k_getpc ());
    return v;
}
...
#endif
    if (GAYLE_LOG)
	printf ("GAYLE_WRITE %08.8X=%02.2X PC=%08.8X\n", addr, (uae_u32)val & 0xff, m68k_getpc ());
    ide_write(addr, val);
}
...
static void mbres_write (uaecptr addr, uae_u32 val, int size)
{
    ...
    if (MBRES_LOG > 0)
	printf ("MBRES_WRITE %08X=%08X (%d) PC=%08X S=%d\n", addr, val, size, m68k_getpc (), regs.s);
...
static uae_u32 mbres_read (uaecptr addr, int size)
{
    ...
    if (MBRES_LOG > 0)
	printf ("MBRES_READ %08X=%08X (%d) PC=%08X S=%d\n", addr, v, size, m68k_getpc (), regs.s);
    return v;
}
...
static void gayle_attr_write (uaecptr addr, uae_u32 v)
{
    ...
    if (addr == 0x40000) {
	if (v)
	    printf ("GAYLE: Reset active\n");
	else
	    printf ("GAYLE: Reset non-active\n");
    }
}
