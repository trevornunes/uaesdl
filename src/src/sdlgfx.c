...
static int find_best_mode (int *width, int *height, int depth, int *try_fs)
{
  ...
  fprintf (stderr, "Function: find_best_mode(%d,%d,%d)\n", *width, *height, depth);
	...
	int i;
	fprintf (stderr,"SDLGFX: Requested mode (%dx%d%d) not available.\n", *width, *height, depth);
...
{
    long count = 0;
    fprintf(stderr,"find_screen_modes\n");
	...
	fprintf(stderr,"SDLGFX: full screen modes found %d", n_fullscreen_modes);
	...
	for (i = count = 0; count < n_fullscreen_modes; i++) {
		...
		fprintf (stderr,"SDLGFX: Found screenmode: %dx%d.\n", w, h);
	    }
	}
    }
}
...
int graphics_setup (void)
{
    ...
    if (SDL_InitSubSystem (SDL_INIT_VIDEO) == 0)
    {
	  ...
	  printf ("SDLGFX: Initialized.\n");
	   printf ("SDLGFX: Using SDL version %d.%d.%d.\n", version->major, version->minor, version->patch);
	...
	result = 1;
	  ...
	  fprintf (stderr,"SDLGFX: initialization failed - %s\n", SDL_GetError());
...
int graphics_subinit (void)
{
    ...
    {
    	fprintf(stderr,"switch back mode\n");
...
#	endif
	...
	if (screen->flags & SDL_DOUBLEBUF) {
		...
		fprintf(stderr,"SDLGFX: no double buffering\n");
	    display = screen;
	}
...
fprintf(stderr,"SDLGFX: SDL_SetColors\n");
