...
#ifdef __QNXNTO__
void gui_display(int i)
{
	printf("gui_display(%d) not supported.\n",i);
}
#endif
...
void inputdevice_handle_inputcode (void)
{
	...
	return;
    if (vpos != 0)
	printf ("inputcode=%d but vpos = %d", code, vpos);
...
static void set_kbr_default (struct uae_prefs *p, int index, int num)
{
	...
	if (j < id->get_num ()) {
		...
		while (trans[l].scancode >= 0) {
			...
			if (k == MAX_INPUT_SUB_EVENT) {
			    printf ("corrupt default keyboard mappings\n");
			    return;
			}
