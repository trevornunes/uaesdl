...
void REGPARAM2 kickmem_lput (uaecptr addr, uae_u32 b)
{
	...
	printf ("Illegal kickmem lput at %08lx\n", addr);
}
...
{
    if (currprefs.illegal_mem)
	printf ("Illegal extendedkickmem lput at %08lx\n", addr);
}

static void REGPARAM2 extendedkickmem_wput (uaecptr addr, uae_u32 b)
{
    if (currprefs.illegal_mem)
	printf ("Illegal extendedkickmem wput at %08lx\n", addr);
}
...
static int kickstart_checksum (uae_u8 *mem, int size)
{
	...
	printf ("Kickstart checksum incorrect. You probably have a corrupted ROM image.\n");
    }
    return 0;
}
...
static int patch_shapeshifter (uae_u8 *kickmemory)
{
    ...
    */
	...
	!memcmp (kickmemory + i, kickshift3, sizeof (kickshift3))) {
	    kickmemory[i + 2] = 0x30;
	    printf ("Kickstart KickShifted @%04.4X\n", i);
	    patched++;
	}
    }
    return patched;
...
static int patch_residents (uae_u8 *kickmemory, int size)
{
	...
	if (kickmemory[i] == 0x4a && kickmemory[i + 1] == 0xfc) {
		...
		while (residents[j]) {
			...
			printf ("KSPatcher: '%s' at %08.8X disabled\n", residents[j], i + base);
...
void memory_init (void)
{
    ...
    if (!load_kickstart ()) {
    	 fprintf(stderr,"memory_init: load_kicktart failed ...\n");
	     init_ersatz_rom (kickmemory);
	     ersatzkickfile = 1;
    }
...
void map_banks (addrbank *bank, int start, int size, int realsize)
{
    ...
    if ((size << 16) < realsize) {
	printf ("Please report to bmeyer@cs.monash.edu.au, and mention:\n");
	printf ("Broken mapping, size=%x, realsize=%x\n", size, realsize);
	printf ("Start is %x\n", start);
	printf ("Reducing memory sizes, especially chipmem, may fix this problem\n");
	abort ();
    }
...
static int addr_valid (char *txt, uaecptr addr, uae_u32 len)
{
	...
	printf ("corrupt %s pointer %x (%d) detected!\n", txt, addr, len);
	return 0;
    }
    return 1;
}
