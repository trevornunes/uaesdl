...
#ifdef DEBUG_DRIVE_ID
    printf ("drive_settype_id: DF%d: set to %s\n", drv-floppy, drive_id_name (drv));
#endif
#ifdef DEBUG_DRIVE_ID
    printf ("drive_settype_id: DF%d: set to %s\n", drv-floppy, drive_id_name (drv));
#endif
...
{
    struct zfile *f = zfile_open (fname, "rb");
    if (f) {
	if (wrprot)
	    *wrprot = 0;
    } else {
...
static int drive_insert (drive *drv, int dnum, const char *fname)
{
  ...
  //  fprintf(stderr,"drive_insert: %d %s",dnum, fname);
	...
	if (drv->num_tracks > MAX_TRACKS)
	    printf ("Your diskfile is too big!\n");
...
*/
	...
	if (drv->cyl < 83)
	    ...
	    printf ("program tried to step over track 83\n");
    }
...
static void drive_motor (drive * drv, int off)
{
	...
	disk_data_used = 0;
#ifdef DEBUG_DRIVE_ID
	printf ("drive_motor: Selected DF%d: reset id shift reg.\n", drv-floppy);
#endif
    }
    if (!drv->motoroff && off) {
...
#ifdef DISK_DEBUG
    printf ("track %d, raw track length %d written (total size %d)\n", track, (ti->bitlen + 7) / 8, ti->len);
#endif
}
...
static int drive_write_adf_amigados (drive * drv)
{
	...
	if (((odd << 1) | even) != chksum || ((id & 0x00ff0000) >> 16) != drv->cyl * 2 + side) {
	    if (drv->filetype == ADF_EXT2)
		return 3;
	    printf ("Disk write: checksum error on sector %d header\n", trackoffs);
	    continue;
	}
	odd = getmfmlong (mbuf);
	even = getmfmlong (mbuf + 2);
	...
	if (chksum) {
	    if (drv->filetype == ADF_EXT2)
		return 4;
	    printf ("Disk write: sector %d, data checksum error\n", trackoffs);
	    continue;
	}
...
static int drive_write_ext2 (drive * drv)
{
    ...
    if (len > ti->len) {
	printf ("disk raw write: image file's track %d is too small (%d < %d)!\n", track, ti->len, len);
	return 0;
    }
...
static void drive_write_data (drive * drv)
{
	...
	break;
    case ADF_EXT1:
	printf ("writing to ADF_EXT1 not supported\n");
	return;
    case ADF_EXT2:
...
void DISK_check_change (void)
{
	...
	if (currprefs.dfxtype[i] != changed_prefs.dfxtype[i]) {
	    drv->type = currprefs.dfxtype[i] = changed_prefs.dfxtype[i];
	    reset_drive (i);
	}
...
#ifdef DISK_DEBUG
		printf ("delayed insert, drive %d, image '%s'\n", i, drv->newname);
#endif
		drv->newname[0] = 0;
	    }
	}
...
uae_u8 DISK_status (void)
{
	...
	if (!((selected | disabled) & (1 << dr))) {
		...
		if (drv->idbit && currprefs.dfxtype[dr] != DRV_35_DD_ESCOM)
		    st &= ~0x20;
#ifdef DEBUG_DRIVE_ID
		printf ("DISK_status: sel %d id %s [0x%08lx, bit #%02d: %d]\n",
...
static void disk_dmafinished (void)
{
    ...
    dskdmaen = 0; /* ??? */
#ifdef DISK_DEBUG
    printf ("disk dma finished %08.8X\n", dskpt);
#endif
}
...
static void disk_doupdate_read (drive * drv)
{
	...
	if (bitoffset == 31 && dma_enable) {
		...
		printf ("Bug: Disk DMA buffer overflow!\n");
		j--;
	    }
	}
...
void DSKLEN (uae_u16 v, int hpos)
{
...
#if 0
	if (dskdmaen == 2)
	    printf ("warning: Disk read DMA aborted, %d words left\n", dsklength);
#endif
	if (dskdmaen == 3)
	    printf ("warning: Disk write DMA aborted, %d words left\n", dsklength);
	dskdmaen = 0;
    }
    dsklength = v & 0x3ffe;
    if (dskdmaen <= 1)
	...
	if (dr == 4)
	    printf ("disk DMA started, but no drive ready!\n");
	else
	    printf ("disk %s DMA started for %d words (%08.8X) sync=%04.4X track %d\n",
...
void DSKDAT (uae_u16 v)
{
    ...
    if (count < 5) {
	count++;
	printf ("DSKDAT written. Not good.");
	if (count == 5)
	    printf ("(further messages suppressed)");
...
void DISK_init (void)
{
    ...
    for (dr = 0; dr < 4; dr++) {
	drive *drv = floppy + dr;
	drv->type = currprefs.dfxtype[dr];
	reset_drive (dr);
    }
    if (disk_empty (0))
	printf ("No disk in drive 0.\n");
    else
    printf("disk drive 0 has disk present.\n");
}
...
const uae_u8 *restore_disk (int num, const uae_u8 *src)
{
    ...
    drv->type = changed_prefs.dfxtype[num];
    drv->buffered_cyl = -1;
    drv->buffered_side = -1;
    drv->cyl = restore_u8 ();
