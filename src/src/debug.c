...
static void illg_debug_do (uaecptr addr, int rw, int size, uae_u32 val)
{
	...
	if (mask & 0x80) {
	    ...
	    if (rw)
		printf ("RW: %08.8X=%02.2X %c PC=%08.8X\n", ad, v, rws, pc);
	    else
		printf ("RW: %08.8X    %c PC=%08.8X\n", ad, rws, pc);
	    if (illgdebug_break)
		activate_debugger ();
	} else if ((mask & 1) && rw) {
	    printf ("RO: %08.8X=%02.2X %c PC=%08.8X\n", ad, v, rws, pc);
	    if (illgdebug_break)
		activate_debugger ();
	} else if ((mask & 2) && !rw) {
	    printf ("WO: %08.8X    %c PC=%08.8X\n", ad, rws, pc);
	    if (illgdebug_break)
		activate_debugger ();
	}
    }
...
static void memwatch (char **c)
{
	...
	if (illgdebug) {
		...
		if (more_params (c))
		    len = readhex (c);
		printf ("cleared logging addresses %08.8X - %08.8X\n", addr, addr + len);
		while (len > 0) {
		    addr &= 0xffffff;
		    illgdebug[addr] = 0;
		    addr++;
...
void debug (void)
{
	...
	printf ("Memwatch %d: break at %08.8X.%c %c %08.8X\n", memwatch_triggered - 1, mwhit.addr,
