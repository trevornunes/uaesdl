...
void SERPER (uae_u16 w)
{
    ...
    if (w&0x8000)
	printf ("SERPER: 9bit transmission not implemented.\n");
    ...
    switch (w & 0x7fff) {
     ...
     default:
	printf ("SERPER: unsupported baudrate (0x%04x) %d\n",w&0x7fff,
	...
	if (tcgetattr (sd, &tios) < 0) {
	    printf ("SERPER: TCGETATTR failed\n");
	    return;
	}

	if (cfsetispeed (&tios, pspeed) < 0) {    /* set serial input speed */
	    printf ("SERPER: CFSETISPEED (%d bps) failed\n", baud);
	    return;
	}
	if (cfsetospeed (&tios, pspeed) < 0) {    /* set serial output speed */
	    printf ("SERPER: CFSETOSPEED (%d bps) failed\n", baud);
	    return;
	}

	if (tcsetattr (sd, TCSADRAIN, &tios) < 0) {
	    printf ("SERPER: TCSETATTR failed\n");
	    return;
	}
    }
#endif
...
#if SERIALDEBUG > 0
    if (serdev == 1)
	printf ("SERPER: baudrate set to %d bit/sec\n", baud);
#endif
}
...
void SERDAT (uae_u16 w)
{
	...
	if (!isbaeh) {
	    printf ("SERDAT: Baeh.. Your software needs SERIAL_ALWAYS to work properly.\n");
	    isbaeh=1;
	}
	return;
    } else {
...
#if SERIALDEBUG > 1
	printf ("SERDATS: received 0x%02x --> serdat==0x%04x\n",
