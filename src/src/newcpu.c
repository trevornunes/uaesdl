...
static void build_cpufunctbl (void)
{
    ...
    if (tbl == 0) {
	printf ("no CPU emulation cores available!");
	abort ();
    }
...
unsigned long REGPARAM2 op_illg (uae_u32 opcode)
{
	...
	printf ("Your Kickstart requires a 68020 CPU. Giving up.\n");
	broken_in = 1;
...
static void mmu_op30_pmove (uaecptr pc, uae_u32 opcode, uae_u16 next, uaecptr extra)
{
	...
	if (siz == 8) {
	    ...
	    if (rw)
		printf ("PMOVE %s,%08X%08X", reg, val2, val);
	    else
		printf ("PMOVE %08X%08X,%s", val2, val, reg);
	} else {
	    if (siz == 4)
		val = get_long (extra);
	    else
		val = get_word (extra);
	    if (rw)
		printf ("PMOVE %s,%08X", reg, val);
	    else
		printf ("PMOVE %08X,%s", val, reg);
	}
	printf (" PC=%08X\n", pc);
    }
#endif
...
static void mmu_op30_ptest (uaecptr pc, uae_u32 opcode, uae_u16 next, uaecptr extra)
{
    ...
    printf ("PTEST%c %02X,%08X,#%X%s PC=%08X\n",
...
void m68k_go (int may_quit)
{
	...
	if (regs.panic) {
		...
		printf ("double bus error or corrupted stack, forcing reboot..\n");
		regs.panic = 0;
		uae_reset (1);
	    }
	}
...
void cpureset (void)
{
	...
	printf ("M68K RESET PC=%x, rebooting..\n", pc);
	customreset ();
	m68k_setpc (ksboot);
	return;
    }
	...
	printf ("reset/jmp (ax) combination emulated -> %x\n", addr);
	customreset ();
	if (addr < 0x80000)
	    addr += 0xf80000;
	m68k_setpc (addr - 2);
