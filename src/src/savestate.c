...
static void restore_header (const uae_u8 *src)
{
    ...
    printf ("Saved with: '%s %s', description: '%s'\n",
...
void restore_state (const char *filename)
{
	...
	printf ("%s is not an AmigaStateFile\n",filename);
	goto error;
    }
    savestate_file = f;
	...
	if (!strcmp (name, "CPU "))
	    ...
	    printf ("unknown chunk '%s' size %d bytes\n", name, len);
	if (len != end - chunk)
	    printf ("Chunk '%s' total size %d bytes but read %d bytes!\n",
		       name, len, end - chunk);
	free (chunk);
    }
    gui_update ();
