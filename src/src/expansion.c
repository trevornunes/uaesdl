...
{
    printf ("expamem_map_clear() got called. Shouldn't happen.\n");
}
...
{
    printf ("warning: READ.L from address $%lx \n", (long unsigned int) addr);
    return 0xfffffffful;
}

static uae_u32 REGPARAM2 expamem_wget (uaecptr addr)
{
    printf ("warning: READ.W from address $%lx \n", (long unsigned int) addr);
    return 0xffff;
}
...
{
    printf ("warning: WRITE.L to address $%lx : value $%lx\n", (long unsigned int) addr, (long unsigned int)value);
}

static void REGPARAM2 expamem_wput (uaecptr addr, uae_u32 value)
{
    if (expamem_type() != zorroIII)
	printf ("warning: WRITE.W to address $%lx : value $%x\n", addr, value);
    else {
	switch (addr & 0xff) {
	 case 0x44:
	    if (expamem_type() == zorroIII) {
...
static void REGPARAM2 expamem_bput (uaecptr addr, uae_u32 value)
{
	...
	break;
     ...
     case 0x4c:
	printf ("   Card %d (Zorro %s) had no success.\n", ecard + 1, expamem_type() == 0xc0 ? "II" : "III");
	++ecard;
	if (ecard < MAX_EXPANSION_BOARDS)
	    (*card_init[ecard]) ();
	else
...
{
    printf ("filesys_lput called\n");
}

static void REGPARAM2 filesys_wput (uaecptr addr, uae_u32 w)
{
    printf ("filesys_wput called\n");
}

static void REGPARAM2 filesys_bput (uaecptr addr, uae_u32 b)
{
    printf ("filesys_bput called. This usually means that you are using\n");
    printf ("Kickstart 1.2. Please give UAE the \"-a\" option next time\n");
    printf ("you start it. If you are _not_ using Kickstart 1.2, then\n");
    printf ("there's a bug somewhere.\n");
    printf ("Exiting...\n");
    uae_quit ();
}
...
static void expamem_map_fastcard (void)
{
    ...
    printf ("Fastcard: mapped @$%lx: %dMB fast memory\n", fastmem_start, allocated_fastmem >> 20);
}
...
static void expamem_map_filesys (void)
{
    ...
    printf ("Filesystem: mapped memory @$%lx.\n", filesys_start);
...
{
	uae_u32 z3fmsize = currprefs.z3fastmem_size;

    z3fastmem_start = ((expamem_hi | (expamem_lo >> 4)) << 16);
    map_banks (&z3fastmem_bank, z3fastmem_start >> 16, z3fmsize >> 16,
	       allocated_z3fastmem);

    printf ("Fastmem (32bit): mapped @$%lx: %d MB Zorro III fast memory \n",
...
static void allocate_expamem (void)
{
	...
	if (allocated_fastmem) {
	    ...
	    if (fastmemory == 0) {
		printf ("Out of memory for fastmem card.\n");
		allocated_fastmem = 0;
	    }
	}
    }
	...
	if (allocated_z3fastmem != currprefs.z3fastmem_size) {
		...
		if (z3fastmem == 0) {
		    printf ("Out of memory for 32 bit fast memory.\n");
		    allocated_z3fastmem = 0;
		}
	    }
	}
	if (allocated_gfxmem != currprefs.gfxmem_size) {
		...
		if (gfxmemory == 0) {
		    printf ("Out of memory for graphics card memory\n");
		    allocated_gfxmem = 0;
		}
	    }
	}
	...
	if (allocated_fastmem > 0) {
		...
		printf ("Statefile most likely corrupt.  Using a sane default for fastmem address.\n");
...
void expamem_reset (void)
{
    ...
    {
	/* warn user */
	printf ("Kickstart version is below 1.3!  Disabling autoconfig devices.\n");
	do_mount = 0;
    }
...
void expansion_init (void)
{
    ...
    allocated_gfxmem = 0;
    allocated_fastmem = 0;
    fprintf(stderr,"expansion_unit\n");

    allocate_expamem ();

    filesysory = mapped_malloc (0x10000, "filesys");
    if (!filesysory) {
	printf ("virtual memory exhausted (filesysory)!\n");
	exit (0);
    }
