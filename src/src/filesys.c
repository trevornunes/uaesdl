			...
			int *cylinders, int *size, int *blocksize, int *bootpri)
{
    ...
    *devname = uip->devname ? my_strdup (uip->devname) : 0;
...
{
    UnitInfo ui = mountinfo->ui[nr];
    char *result = set_filesys_unit_1 (mountinfo, nr, devname, volname, rootdir,
				       readonly, secspertrack, surfaces, reserved,
				       blocksize, bootpri);
    if (result)
	mountinfo->ui[nr] = ui;
    else
	...
	while (i < 50) {
		...
		for (;;) {
			...
			if (aino->shlock > 0 || aino->elock)
			    printf ("panic: freeing locked a_inode!\n");
...
static char *create_nname (Unit *unit, a_inode *base, char *rel)
{
	...
	printf ("Filesystem in trouble... please report.\n");
	free (p);
	goto oh_dear;
    }
    return p;
...
static void init_child_aino (Unit *unit, a_inode *base, a_inode *aino)
{
	...
	printf ("Running out of a_inodes (prepare for big trouble)!\n");
    }
    aino->shlock = 0;
    aino->elock = 0;
...
static uae_u32 startup_handler (TrapContext *dummy)
{
	...
	*s = '\0';
    ...
    {
	printf ("Failed attempt to mount device\n", devname);
...
static Key *lookup_key (Unit *unit, uae_u32 uniq)
{
    ...
    printf ("Error: couldn't find key!\n");
#if 0
    exit(1);
    /* NOTREACHED */
#endif
    ...
    printf ("Better reset that Amiga - the system is messed up.\n");
    return 0;
}
...
static void move_exkeys (Unit *unit, a_inode *from, a_inode *to)
{
	...
	printf ("filesys.c: Bug in ExNext bookkeeping.  BAD.\n");
...
static void action_examine_next (Unit *unit, dpacket packet)
{
    ...
    uniq = get_long (info);
    if (uniq == 0) {
	printf ("ExNext called for a file! (Houston?)\n");
	goto no_more_entries;
    ...
    if (ek == 0) {
	printf ("Couldn't find a matching ExKey. Prepare for trouble.\n");
	goto no_more_entries;
    }
...
action_read (Unit *unit, dpacket packet)
{
	...
	char *buf;
	printf ("unixfs warning: Bad pointer passed for read: %08x\n", addr);
...
action_delete_object (Unit *unit, dpacket packet)
{
    ...
    if (a->child != 0) {
	printf ("Serious error in action_delete_object.\n");
    } else {
	delete_aino (unit, a);
    }
    PUT_PCK_RES1 (packet, DOS_TRUE);
...
static uae_u32 exter_int_helper (TrapContext *dummy)
{
...
#else
	printf ("exter_int_helper should not be called with arg 1!\n");
#endif
	break;
     case 2:
	...
	while (comm_pipe_has_data (&native2amiga_pending)) {
		...
		return 3;
	     ...
	     default:
		printf ("exter_int_helper: unknown native action\n");
		break;
	    }
	}
#endif
	...
	break;
     ...
     default:
	printf ("Shouldn't happen in exter_int_helper.\n");
	break;
    }
    return 0;
}
...
static uae_u32 filesys_handler (TrapContext *dummy)
{
	...
	printf ("Bad address passed for packet.\n");
	goto error2;
    }
    ...
    do_put_mem_long ((uae_u32 *)(msg + 4), -1);
    if (!unit || !unit->volume) {
	printf ("Filesystem was not initialized.\n");
	goto error;
    }
#ifdef UAE_FILESYS_THREADS
    {
...
static int rdb_checksum (char *id, uae_u8 *p, int block)
{
    ...
    sum = -sum;
    if (sum) {
	printf ("RDB: block %d ('%s') checksum error\n", block, id);
	return 0;
    }
    return 1;
}
...
static void dump_partinfo (char *name, int num, uaecptr pp, int partblock)
{
    ...
    printf ("RDB: '%s' dostype=%08.8X. PartBlock=%d\n", name, dostype, partblock);
    printf ("BlockSize: %d, Surfaces: %d, SectorsPerBlock %d\n",
	       get_long (pp + 20) * 4, get_long (pp + 28), get_long (pp + 32));
    printf ("SectorsPerTrack: %d, Reserved: %d, LowCyl %d, HighCyl %d\n",
	       get_long (pp + 36), get_long (pp + 40), get_long (pp + 52), get_long (pp + 56));
    printf ("Buffers: %d, BufMemType: %08.8x, MaxTransfer: %08.8x, BootPri: %d\n",
...
#define rdbmnt printf ("Mounting uaehf.device %d (%d) (size=%llu):\n", unit_no, partnum, hfd->size);
...
static int rdb_mount (UnitInfo *uip, int unit_no, int partnum, uaecptr parmpacket)
{
...
#if 0
	    if (rdb_checksum ("RDSK", bufrdb, rdblock)) {
		printf ("Windows 95/98/ME trashed RDB detected, fixing..\n");
    ...
    if (badblock != -1) {
	rdbmnt
	printf ("RDB: badblock list is not yet supported. Contact the author.\n");
	return -2;
    }
    driveinitblock = rl (bufrdb + 36);
    if (driveinitblock != -1) {
	rdbmnt
	printf ("RDB: driveinit is not yet supported. Contact the author.\n");
	return -2;
    }
    ...
    if (partnum == 0) {
	printf ("RDB: RDSK detected at %d, FSHD=%d, C=%d S=%d H=%d\n",
	...
	err = -1;
	printf ("RDB: Automount disabled, not mounting\n");
	goto error;
    }
    ...
    if (dostype == 0) {
	printf ("RDB: mount failed, dostype=0\n");
	err = -1;
	goto error;
    }

    if (hfd->cylinders * hfd->sectors * hfd->heads * blocksize > hfd->size)
	printf ("RDB: WARNING: end of partition > size of disk!\n");
    ...
    if (!fsres) {
	printf ("RDB: FileSystem.resource not found, this shouldn't happen!\n");
	goto error;
    }
    fsnode = get_long (fsres + 18);
    while (get_long (fsnode)) {
	...
	if (fileblock == -1) {
	    if (!fsnode)
		printf ("RDB: required FS %08.8X not in FileSystem.resource or in RDB\n", dostype);
	    goto error;
	}
	if (!legalrdbblock (uip, fileblock)) {
	    printf ("RDB: corrupt FSHD pointer %d\n", fileblock);
	    goto error;
	}
	memset (buf, 0, readblocksize);
	hdf_read (hfd, buf, fileblock * hfd->blocksize, readblocksize);
	if (!rdb_checksum ("FSHD", buf, fileblock)) {
	    printf ("RDB: checksum error in FSHD block %d\n", fileblock);
	    goto error;
	}
	fileblock = rl (buf + 16);
    ...
    printf ("RDB: RDB filesystem %08.8X version %d.%d\n", dostype, newversion, newrevision);
    if (fsnode) {
	printf ("RDB: %08.8X in FileSystem.resouce version %d.%d\n", dostype, oldversion, oldrevision);
    }
    if (newversion * 65536 + newrevision <= oldversion * 65536 + oldrevision && oldversion >= 0) {
	printf ("RDB: FS in FileSystem.resource is newer or same, ignoring RDB filesystem\n");
	goto error;
    }
    ...
    printf ("RDB: Filesystem loaded, %d bytes\n", i * (blocksize - 20));
			   ...
			   uaecptr *devname_amiga, int unit_no)
{
    ...
    *devname_amiga = ds (*devname);
    if (type == FILESYS_VIRTUAL)
	printf ("FS: mounted virtual unit %s\n", buffer);
    else
	printf ("FS: mounted HDF unit %s\n", buffer);
}
