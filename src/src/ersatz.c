...
{
    printf ("Trying to use Kickstart replacement.\n");
...
static void ersatz_doio (void)
{
	...
	break;
     ...
     default:
	printf ("Only CMD_READ supported in DoIO()\n");
	ersatz_failed ();
    }
    {
...
static void ersatz_init (void)
{
    ...
    if (disk_empty (0)) {
	printf ("You need to have a diskfile in DF0 to use the Kickstart replacement!\n");
	uae_quit ();
	m68k_setpc (0xF80010);
	return;
    }
	...
	printf ("Loading Kickstart rom image from Kickstart disk\n");
	/* print some notes... */
	printf ("NOTE: if UAE crashes set CPU to 68000 and/or chipmem size to 512KB!\n");
