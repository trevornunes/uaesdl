...
static int bplres, adjusted_bplres;
...
void init_row_map (void)
{
	...
	printf ("Resolution too high, aborting\n");
	abort ();
    }
...
STATIC_INLINE void pfield_draw_line (int lineno, int gfx_ypos, int follow_ypos)
{
	...
	if (!warned)
	    printf ("Shouldn't get here... this is a bug.\n"), warned++;
	return;
...
static void draw_status_line (int line)
{
	...
	if (y >= TD_PADY && y - TD_PADY < TD_NUM_HEIGHT) {
	    if (num3 >= 0) {
		int tn = num1 > 0 ? 3 : 2;
		x += (TD_LED_WIDTH - tn * TD_NUM_WIDTH) / 2;
		if (num1 > 0) {
...
void finish_drawing_frame (void)
{
	...
	if (where >= gfxvidinfo.height - (curr_gfx->leds_on_screen ? TD_TOTAL_HEIGHT : 0))
	    break;
	if (where == -1)
	    continue;
...
int check_prefs_changed_gfx (void)
{
    ...
    return params_changed || screen_changed;
}
...
void vsync_handle_redraw (int long_frame, int lof_changed)
{
	...
	currprefs.keyboard_lang = changed_prefs.keyboard_lang;
...
void reset_drawing (void)
{
    ...
    if (!curr_gfx)
    {
     fprintf(stderr,"reset_drawing: curr_gfx null.\n");
     return;
    }
