...
{
    fprintf (f, "gfx_width_%s=%d\n", suffix, p->width);
    fprintf (f, "gfx_height_%s=%d\n", suffix, p->height);
    fprintf (f, "gfx_lores_%s=%s\n", suffix, p->lores ? "true" : "false");
    fprintf (f, "gfx_linemode_%s=%s\n", suffix, linemode1[p->linedbl]);
    fprintf (f, "gfx_correct_aspect_%s=%s\n", suffix, p->correct_aspect ? "true" : "false");
    fprintf (f, "gfx_center_horizontal_%s=%s\n", suffix, centermode1[p->xcenter]);
    fprintf (f, "gfx_center_vertical_%s=%s\n", suffix, centermode1[p->ycenter]);
    fprintf (f, "gfx_show_leds_%s=%s\n", suffix, p->leds_on_screen ? "true" : "false");
}
...
void save_options (FILE *f, const struct uae_prefs *p)
{
	...
	fprintf (f, "kickstart_rom_crc=0x%x\n", currprefs.rom_crc32);
...
int cfgfile_yesno (const char *option, const char *value, const char *name, int *location)
{
	...
	*location = 0;
	...
	printf ("Option `%s' requires a value of either `yes' or `no'.\n", option);
    return 1;
}
...
int cfgfile_intval (const char *option, const char *value, const char *name, int *location, int scale)
{
    ...
    *location = strtol (value, &endptr, base) * scale;
    ...
    if (*endptr != '\0' || *value == '\0')
	printf ("Option `%s' requires a numeric argument.\n", option);
    return 1;
}
...
int cfgfile_strval (const char *option, const char *value, const char *name, int *location, const char *table[], int more)
{
	...
	printf ("Unknown value for option `%s'.\n", option);
	return 1;
    }
    *location = val;
    return 1;
...
static int cfgfile_parse_host (struct uae_prefs *p, char *option, char *value)
{
	...
	*tmpp = '\0';
	...
	if (p->sound_stereo == SND_MONO) {
	    printf ("Mono sound no longer supported, switching to stereo.\n");
	...
	if ((l = KBD_LANG_DE, strcasecmp (value, "de") == 0)
	    ...
	    printf ("Unknown keyboard language\n");
	return 1;
    }
	...
	*tmpp++ = '\0';
	...
	return 1;
      ...
      invalid_fs:
	printf ("Invalid filesystem/hardfile specification.\n");
	return 1;
    }
...
void cfgfile_parse_line (struct uae_prefs *p, char *line)
{
    ...
    line2 = strchr (line, '=');
    if (! line2) {
	printf ("CFGFILE: line was incomplete with only %s\n", line1);
	return;
    }
...
static void parse_joy_spec (struct uae_prefs *p, char *spec)
{
    ...
    if (0)
bad:
    printf ("Bad joystick mode specification. Use -J xy, where x and y\n"
...
int parse_cmdline_option (struct uae_prefs *p, char c, char *arg)
{
	...
	if (strchr (arg, 'S') != NULL || strchr (arg, 's')) {
	    printf ("  Serial on demand.\n");
	    p->serial_demand = 1;
	}
	if (strchr (arg, 'P') != NULL || strchr (arg, 'p')) {
	    printf ("  Parallel on demand.\n");
	    p->parallel_demand = 1;
	}
